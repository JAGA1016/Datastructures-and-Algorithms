...
elements of the part of the array that is required to be sorted [integer type]*/
{
    while (right >left) 
    {
        ...
        and storing the value of index of first occurence of pivot value after partitioning in variable iof*/
        quick_sort (arr, left, pi-1); //recursively calling quick_sort function to sort part of array that is left to iof
        left = pi+1; //reinitializing left to pi+1 so that part of array right to pi gets sorted
    }
}
